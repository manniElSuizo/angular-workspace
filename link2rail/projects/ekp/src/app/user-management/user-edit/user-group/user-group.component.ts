...
import { UserListService, UserGroupListResponse, RolesListResponse } from '../../user-list/user-list.service';
...
export class UserGroupComponent implements AfterViewInit, OnInit {
  ...
  filterGroups(limit?: number) {
    ...
    let assingedGroupIds = this.user.groups.map((g) => g.groupId);
  ...
  updateGroupList() {
    this.closeDialog();
    this.addedGroups.forEach((g) => this.user.groups.push(g));
    this.user.groups.sort((a, b) => this.getCustomer(a.customerProfile).localeCompare(this.getCustomer(b.customerProfile)) ?? a.groupName.localeCompare(b.groupName));
    this.user.authorizationsChanged = true;
  }
